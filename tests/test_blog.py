...
''' 
    all of the blog views use the auth fixture. Call auth.login()
    and subsequent requests from the client will be logged in as the 
    test user.
    ...
    You can also test some more authentication behaviour while testing the
    index view. When not logged in, each page shows links to log in or register.
...
''' 
    ...
    otherwise a 403 Forbidden status is returned. If a post with the given id doesn't
    exist, update and delete should return a 404 Not Found.
'''
...
''' 
    ...
    should insert the new post data into the database, update should
    modify the existing data. Both pages should show an error message 
    on invalid data.
'''
